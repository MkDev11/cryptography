...
This module provides a hazmat API for HPKE, which combines asymmetric
key encapsulation with symmetric authenticated encryption.

Only Base mode is supported. This initial implementation supports:
...
# Suite IDs for the supported configuration
_KEM_ID = 0x0020  # DHKEM(X25519, HKDF-SHA256)
_KDF_ID = 0x0001  # HKDF-SHA256
_AEAD_ID = 0x0001  # AES-128-GCM

# KEM parameters for X25519
_NSECRET = 32
_NENC = 32
_NPK = 32
_NSK = 32
...
def _i2osp(n: int, length: int) -> bytes:
    """Integer to Octet String Primitive (I2OSP) as defined in RFC 8017."""
...
_HPKE_SUITE_ID = (
    b"HPKE" + _i2osp(_KEM_ID, 2) + _i2osp(_KDF_ID, 2) + _i2osp(_AEAD_ID, 2)
)
...
def _kem_labeled_extract(salt: bytes, label: bytes, ikm: bytes) -> bytes:
    ...
    return hkdf._extract(labeled_ikm)  # type: ignore[attr-defined]
...
def _kem_labeled_expand(
    ...
    labeled_info = (
        _i2osp(length, 2) + _HPKE_VERSION + _KEM_SUITE_ID + label + info
    )
...
def _extract_and_expand(dh: bytes, kem_context: bytes) -> bytes:
    ...
    shared_secret = _kem_labeled_expand(
        eae_prk, b"shared_secret", kem_context, _NSECRET
    )
...
def _hpke_labeled_expand(
    ...
    labeled_info = (
        _i2osp(length, 2) + _HPKE_VERSION + _HPKE_SUITE_ID + label + info
    )
    hkdf_expand = HKDFExpand(
        algorithm=hashes.SHA256(),
        length=length,
...
def _key_schedule(shared_secret: bytes, info: bytes) -> tuple[bytes, bytes]:
    ...
    key = _hpke_labeled_expand(secret, b"key", key_schedule_context, _NK)
    base_nonce = _hpke_labeled_expand(
        secret, b"base_nonce", key_schedule_context, _NN
    )
...
class SenderContext:
    ...
    def _compute_nonce(self) -> bytes:
        ...
        seq_bytes = _i2osp(self._seq, len(self._base_nonce))
